...
class TeensySPISettings{
	public:
		#if defined(MAIN_PROCESSOR)
			#if defined(USING_KINETIS)
				TeensySPISettings(uint32_t clock, uint8_t bitOrder, uint8_t dataMode){
				...
				TeensySPISettings(uint32_t clockIn, uint8_t bitOrderIn, uint8_t dataModeIn) : _clock(clockIn) {
					init_AlwaysInline(bitOrderIn, dataModeIn);
				}

				TeensySPISettings() : _clock(DEFAULT_SPEED) {
					init_AlwaysInline(MSBFIRST, SPI_MODE0);
				}

			#endif
		#elif defined(EXTERNAL_PROCESSOR) & defined(USING_KINETIS)
			TeensySPISettings(uint8_t bitOrderIn, uint8_t dataModeIn) {
				init_AlwaysInline(bitOrderIn, dataModeIn);
			}
...
class TeensySPI{
	public:
		...
		//Constructor
		constexpr TeensySPI(uintptr_t myport, uintptr_t myhardware) 
		...
		void configureSPIsettings(TeensySPISettings *settings) {_moduleSettings = settings; }
	...
	private:
		...
		TeensySPISettings *_moduleSettings = nullptr;
